...
namespace BankApp_Inheritance_.UI
{
    class ClientClassUI
    {
        ...
        public static void checkBalance()
        {
            ...
            Console.WriteLine("Your Current Balance is: " + users.getCash());
            Console.WriteLine("Your Current Loan to Pay is: " + users.getLoans());
        ...
        public static void cashWithdraw()
        {
            ...
            else if (int.Parse(withdraw) > users.getCash())
            {
        ...
        public static void cashDeposit()
        {
            ...
            if (creditCard == users.getCreditCardNo())
            {
            ...
            else if (users.getLoans() == 0)
            {
                ...
                if (loan >= 1000000)
                {
                    if (sum <= 360)
                    {
                        ...
                        users.setCash(users.getCash() + loan);
                        users.setLoans(loan);
        ...
        public static void loanPay()
        {
            ...
            else if (users.getLoans() > 0)
            {
                ...
                while (check == false)
                {
                    ...
                    BankClassUI.header();
                    userClientMenu("Pay Loans");
                    Console.WriteLine("You Have " + users.getLoans() + " Rs to Pay");
                ...
                if (creditCard == users.getCreditCardNo() && users.getCash() > int.Parse(payLoan))
                {

                    if (int.Parse(payLoan) >= users.getLoans())
                    {
                        AdminClassDL.addBankBalance(int.Parse(payLoan));

                        int payed;
                        payed = int.Parse(payLoan) - users.getLoans();
                        users.setCash(users.getCash() + payed);
                        users.setLoans(0);
                        Console.WriteLine("You have Paid more than you Were owed, Your dept has been payed");
                        Console.WriteLine("Remaining Money i.e. " + payed + " Rs. has been Deposited to your Account ");

                        BankClassUI.press();
                    }
                    else if (int.Parse(payLoan) < users.getLoans())
                    {
                        AdminClassDL.addBankBalance(int.Parse(payLoan));

                        users.setLoans(users.getLoans() - int.Parse(payLoan));
                        users.setCash(users.getCash() - int.Parse(payLoan));
                        Console.WriteLine("Now you are Owed " + users.getLoans() + " Rs. More");
                        BankClassUI.press();
                    }
                }
            }
        ...
        public static void sendCash()
        {
            ...
            else if (reciever != -1)
            {
                ...
                if (users.getCash() >= sendMoney)
                {
                    users.setCash(users.getCash() - sendMoney);
                    userR.setCash(userR.getCash() + sendMoney);

                    Console.WriteLine("Your Remaining Balance is: " + users.getCash());
                    ClientClassDL.storeData();

                }
                else if (users.getCash() < sendMoney)
                {
        ...
        public static void complaint()
        {
            ...
            users.setComplaints(Console.ReadLine());

            users.setComplaintCheck(true);
        ...
        public static void editAccount()
        {
            ...
            users.setName(BankClassUI.signUpName("Main Menu > Login Menu > Clint Menu > Edit Account"));

            users.setPassword(BankClassUI.signUpPassword(users.getName(), "Main Menu > Login Menu > Clint Menu > Edit Account"));
        ...
        public static void clientFunds()
        {
            ...
            if (creditCard == users.getCreditCardNo() && users.getCash() > fund)
            {
                ...
                users.setFunds(true);
                users.setFundPayed(users.getFundPayed() + fund);
                ClientClassDL.storeData();

            }
            else if (creditCard != users.getCreditCardNo() || users.getCash() < fund)
            {
        ...
        public static void otherServices()
        {
            ...
            if (choice == "1")
            {
                if (users.getCheckBook() == false)
                {
                    users.setCheckBook(true);
                    Console.WriteLine("Your Request Has Been Sent: ");
                    ClientClassDL.storeData();

                }
                else if (users.getCheckBook() == true)
                {
            ...
            {
                if (users.getNewCard() == false)
                {
                    users.setNewCard(true);
                    Console.WriteLine("Your Request Has Been Sent: ");
                    ClientClassDL.storeData();

                }
                else if (users.getNewCard() == true)
                {
        ...
        public static bool clientReview(string choice)
        {
            ...
            {
                check = true;
                users.setReviewCheck(true);
                users.setClientReviews("Excellent");
            }
            else if (choice == "2")
            {
                check = true;
                users.setReviewCheck(true);
                users.setClientReviews("Very Good");
            }
            else if (choice == "3")
            {
                check = true;
                users.setReviewCheck(true);
                users.setClientReviews("Good");
            }
            else if (choice == "4")
            {
                check = true;
                users.setReviewCheck(true);
                users.setClientReviews("Average");
            }
            else if (choice == "5")
            {
                check = true;
                users.setReviewCheck(true);
                users.setClientReviews("Bad");
            }
            else if (choice == "6")
            {
                check = true;
                users.setReviewCheck(true);
                users.setClientReviews("Very Bad");
            }
            else if (choice == "7")
            {
                check = true;
                users.setReviewCheck(true);
                users.setClientReviews("Worst");
            }
            else
            {
                check = false;
                users.setReviewCheck(false);
                Console.WriteLine("Please Choose a valid Option");
                BankClassUI.press();
            }
            return check;
